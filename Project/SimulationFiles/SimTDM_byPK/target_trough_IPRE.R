...
# Then sample IPRE at the end of the first cycle
# Calculate new dose based on sampled IPRE, target trough of 50 ng/mL and
...
# Set working directory to project output directory
  ...
  # Create a folder in the directory specific to this dosing regimen
    ...
    setwd(study.dir)
# For each individual:
  target.trough.ipre <- function(input.pk.data) {
  ...
  # Sample total measured sunitinib concentration at day 28
    ...
    sample.IPRE <- cyc1.data$IPRE[cyc1.data$time == sample.time]
  # Calculate new dose for the individual
    new.dose <- 50/sample.IPRE*dose	# Where 50 is the target trough, dose = 50 mg
...
# Simulate for the population
  pk.data <- ddply(input.pk.data, .(SIM,ID), target.trough.ipre,
    .progress = "text")
